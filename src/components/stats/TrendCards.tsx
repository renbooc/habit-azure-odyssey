...
export const TrendCards = ({
  ...
  const fetchData = async () => {
    ...
    try {
      let endpoint = "";
      if (view === "daily")
        endpoint = `/stats/history?family_id=${familyId}&username=${username || ""}`;
      else if (view === "weekly")
        endpoint = `/stats/history/weeks?family_id=${familyId}&username=${username || ""}`;
      else if (view === "monthly")
        endpoint = `/stats/history/months?family_id=${familyId}&username=${username || ""}`;
  ...
  return (
    <Card className="p-0 overflow-hidden border-none shadow-ambient bg-surface">
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          <div className="bg-surface-container-low p-4 rounded-2xl border border-outline/5 relative group overflow-hidden">
            <div className="relative z-10">
              ...
              <h4 className="text-2xl font-black text-on-surface italic">
                {totalCount}{" "}
                <span className="text-sm not-italic opacity-40 ml-1">项</span>
              </h4>
            </div>
      ...
      <div className="h-64 w-full px-2 min-w-0 min-h-0">
        {loading ? (
