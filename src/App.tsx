...
export default function App() {
  const { user, role, points, login, logout, refreshPoints, updateAvatar } =
    useUser();
      ...
      <div className="min-h-screen bg-background flex flex-col">
        ...
        <main className="flex-grow px-6 py-8 max-w-2xl mx-auto w-full pb-32">
          <AnimatePresence mode="wait">
            ...
            >
              {screen === "dashboard" && (
                <ChildDashboard onSelectTask={handleSelectTask} />
              )}
              {screen === "admin" && role === "parent" && (
                <ParentDashboard onNavigate={setScreen} />
              )}
              {screen === "tasks" && (
                <Tasks role={role} onSelectTask={handleSelectTask} />
              )}
