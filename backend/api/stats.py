...
from service.trust_service import get_points_multiplier
...
def get_child_stats(family_id: str, username: str):
    ...
    try:
        ...
        # 惩罚类直接削弱总阅历(历史经验值)
        penalty_spent = sum(
            safe_int(p.get("price"))
            for p in purchases
            if str(p.get("item_id", "")).startswith("罚单_")
        )
        ...
        # ── 信任指数：积分倍率 ──
        trust_multiplier = get_points_multiplier(username)
        effective_points = int(points * trust_multiplier)
        effective_total_xp = int(total_xp * trust_multiplier)

        return {
            "level": lvl_info["level"],
            "level_title": lvl_info["title"],
            "level_emoji": lvl_info["emoji"],
            "total_xp": effective_total_xp,
            "points": effective_points,
            "streak_days": streak,
            "plants_count": len(completed_tasks),
            "water_drops": effective_total_xp,  # 信任倍率影响后的水滴
...
def get_parent_stats(family_id: str):
    ...
    try:
        ...
        rate = (
            int((completed_tasks_total / total_tasks * 100)) if total_tasks > 0 else 0
        )
            ...
            if t.get("completed") and t.get("completed_at"):
                ...
                try:
                    ...
                    if dt_local >= monday_of_this_week:
                        ...
                        weekly_data[day_idx][uname] = (
                            weekly_data[day_idx].get(uname, 0) + 1
                        )
    ...
    try:
        ...
        for p in purchases_res.data or []:
            if p.get("created_at"):
                ...
                if raw_item_id.startswith("撤回加分") or raw_item_id.startswith(
                    "撤回扣分"
                ):
